...
MePort stpB(PORT_1);
MePort stpA(PORT_2);
MePort servoPort(PORT_4);
MeDCMotor laser(M2);
int servopin =  servoPort.pin2();
int servopin2 =  servoPort.pin1();
...
void parsePen(char * cmd)
{
  ...
  if (tpos > lastpenpos) {
    for (pos = lastpenpos; pos < tpos; pos += 1) // goes from 0 degrees to 180 degrees
    { // in steps of 1 degree
      servoPen.write(pos);              // tell servo to go to position in variable 'pos'
...
void parseTilt(char * cmd)
{
  ...
  if (tpos > lasttiltpos) {
    for (pos = lasttiltpos; pos < tpos; pos += 1) // goes from 0 degrees to 180 degrees
    { // in steps of 1 degree
      servoTilt.write(pos);              // tell servo to go to position in variable 'pos'
      delay(5);                       // waits 15ms for the servo to reach the position
    }
  }
...
void echoRobotSetup()
{
  ...
  Serial.print(" D"); Serial.print((int)roboSetup.data.penDownPos);
  Serial.print(" d"); Serial.print((int)roboSetup.data.penDrawPos);
  Serial.print(" p"); Serial.println((int)roboSetup.data.penPumpPos);
...
void parsePenPosSetup(char * cmd)
{
  ...
  while (str != NULL) {
    ...
    if (str[0] == 'U') {
      ...
      roboSetup.data.penPumpPos = atoi(str + 1);
    }
  }
...
void initRobotSetup()
{
  ...
  if (strncmp(roboSetup.data.name, "SPIDER1", 7) != 0) {
    ...
    roboSetup.data.penUpPos = 145;
    roboSetup.data.penDownPos = 162;
    roboSetup.data.penDrawPos = 118;
    roboSetup.data.penPumpPos = 142;
    syncRobotSetup();
  }
